...
def get_mapping(maping_file_path):
    ...
    df = pd.read_csv(maping_file_path, sep=" ", header=None,
...
# cluster_labels就是req_c，由FINCH得到的
def estimate_cost_matrix(gt_labels, cluster_labels):
    ...
    for i in L_pred:
        # 将符合条件的元素坐标进行返回，一个类一个类的判断，每个在pred类中的动作所在预测中的位置
        ...
        for j in L_gt:
            ...
            profit_mat[i][j] = np.count_nonzero(
                gt_selected == j)  # 统计非0个数，即统计True的个数
