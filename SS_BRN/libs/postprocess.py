...
class PostProcessor(object):
    ...
    def _convert2label(self, x: np.ndarray) -> np.ndarray:
        assert x.ndim == 2 or x.ndim == 3  # 都对维度有个选择的，我可以先改变一下输入的维度，从一维以最简单的方式变为二维
    ...
    def _refinement_with_boundary(
        ...
        preds = self._convert2label(outputs)  # 改变outputs，有自动改变维度的，但是我需要增加维度
        boundaries = self._convert2probability(
            boundaries)  # boundaries不用变，mask应该也是不用变的
    ...
    def __call__(self, outputs, **kwargs: np.ndarray) -> np.ndarray:
        preds, boundarys = self.func[self.name](outputs, **kwargs)
        return preds, boundarys
