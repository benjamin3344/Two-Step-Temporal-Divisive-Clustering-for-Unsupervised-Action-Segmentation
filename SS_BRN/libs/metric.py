...
class ScoreMeter(object):
    def __init__(
        ...
        ignore_index: int = 255,#为什么要有ignore
    ) -> None:
    ...
    def update(
        ...
        elif len(outputs.shape) == 2:#输入的形式可以为两种
            preds = copy.copy(outputs)

        for pred, gt in zip(preds, gts):#这个地方调用了gt，使用TW出来的结果直接是preds和gts，for x,y in zip(x,y)表示一个一个得取出来
            pred = pred[gt != self.ignore_index]#为什么要ignore_index，要无视某一帧,但是gt也是帧数，下采样后也是6000帧
    ...
    def get_scores(self) -> Tuple[float, float, float]:
        ...
        # accuracy
        acc = 100 * float(self.n_correct) / self.n_frames#一次性计算test10个视频，就是n_frames大约是100000帧
