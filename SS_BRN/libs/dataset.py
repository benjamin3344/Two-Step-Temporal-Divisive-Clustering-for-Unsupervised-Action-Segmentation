...
class ActionSegmentationDataset(Dataset):
    ...
    def __init__(
        ...
        """
            Args:
                dataset: the name of dataset (50Salads, Breakfast, MPII_Cooking)
        ...
        ), "You have to choose 50Saladas, Breakfast, MPII_Cooking as dataset."
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        feature_path = self.df.iloc[idx]["feature"]  # smoothing IDT features
        label_path = self.df.iloc[idx]["label"]  # pseudo labels
        boundary_path = self.df.iloc[idx]["boundary"]

        feature = np.load(feature_path).astype(np.float32)

        if(feature.shape[0] != 2048):
            # For IDT features, transposition is required, while for I3D features, it is not necessary.
            feature = feature.T
        ...
        # Using gt labels to obtain scores and training with pseudo labels.
        ds_name = feature_path.split("/")[2]
        gt_path = os.path.join("./dataset", ds_name, "gt_labels",
                               label_path.split("/")[-1])
        gt = np.load(gt_path).astype(np.int64)
        gt = torch.from_numpy(gt).long()

        if self.transform is not None:
            feature, label, boundary = self.transform(
...
def collate_fn(sample: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for s in sample:
        ...
        if pad_t > 0:
            ...
            gt = F.pad(gt, (0, pad_t), mode="constant", value=255)
