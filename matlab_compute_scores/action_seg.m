...
for video=1:numel(files)
       [vid_feats, gt_label_frame, vid_name]= read_video(files, data_root, video, Dataset);
        ...
        %writeNPY(f_c,"D:\daima\data\MPII_Cooking\req_matlab\" + vid_name +".npy");
        
        %writematrix( f_c,"D:\daima\data\MPII_Cooking\req_matlab\" + vid_name +".txt" , 'Delimiter', ' ');

        f_path = fullfile(data_root, Dataset ,"req_asrf",vid_name)+'.txt';
        f_c = dlmread(f_path);
